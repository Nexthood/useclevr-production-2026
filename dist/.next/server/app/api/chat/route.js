...
Explain to the user that the computation could not be completed and suggest they try a different question.`}},84076:(a,b,c)=>{c.a(a,async(a,d)=>{try{c.r(b),c.d(b,{POST:()=>y});var e=c(5608),f=c(37920),g=c(37726),h=c(46564),i=c(22057),j=c(36806),k=c(77142),l=c(42392),m=c(25074),n=c(6796),o=a([f,h,l]);function p(a){if(!a)return a;for(let b of(a=(a=(a=a.replace(/(\d+\.\d{3,})%/g,a=>parseFloat(a).toFixed(2)+"%")).replace(/(\d+\.\d{3,})(\s*%)/g,(a,b,c)=>parseFloat(b).toFixed(2)+c)).replace(/\b(\d{4,})\b/g,a=>a.startsWith("19")||a.startsWith("20")||a.includes(",")?a:Number(a).toLocaleString("en-US")),[{from:"group_by",to:"grouped by"},{from:"dataset rows",to:"items"},{from:"records",to:"items"},{from:"aggregation",to:"analysis"},{from:"SQL",to:""},{from:"select",to:""}]))a=a.replace(RegExp(b.from,"gi"),b.to);return a}async function q(a,b){(0,e.cY)("[STRICT_SQL] Generating SQL for question:",b);let c=await h.db.query.datasets.findFirst({where:(0,m.eq)(i.datasets.id,a)});if(!c)return{success:!1,error:"Dataset not found"};let d=c.data||[],f=c.columns||[];if(0===d.length)return{success:!1,error:"Dataset has no data"};(0,e.cY)("[STRICT_SQL] Dataset:",c.name,"- Rows:",d.length,"- Columns:",f.length);let g=b.toLowerCase(),j="",k=null,l=a=>f.find(b=>a.some(a=>b.toLowerCase().includes(a)));try{if(g.includes("how many row")||g.includes("count row")||g.includes("number of row"))j="SELECT COUNT(*) as count FROM dataset",k={count:d.length,operation:"count"};else if(g.includes("total")||g.includes("sum")||g.includes("revenue")||g.includes("sales")){let a=l(["revenue","sales","amount","total","price","cost"]);if(a){let b=d.reduce((b,c)=>b+(parseFloat(c[a])||0),0);j=`SELECT SUM(${a}) as total FROM dataset`,k={total:b,column:a,operation:"sum"}}}else if(g.includes("average")||g.includes("avg")||g.includes("mean")){let a=l(["revenue","sales","amount","price","cost","profit"]);if(a){let b=d.map(b=>parseFloat(b[a])||0),c=b.reduce((a,b)=>a+b,0)/b.length;j=`SELECT AVG(${a}) as average FROM dataset`,k={average:c,column:a,operation:"avg"}}}else if(g.includes("region")||g.includes("country")||g.includes("product")||g.includes("channel")||g.includes("segment")||g.includes("category")||g.includes("highest")||g.includes("lowest")||g.includes("most")||g.includes("top")||g.includes("best")||g.includes("worst")||g.includes("least")||g.includes("brings")||g.includes("generates")||g.includes("produces")){let a=l(["region","country","product","category","segment","channel","source","medium","campaign","customer","industry","area","zone"]),b=l(["revenue","sales","profit","amount","total","value","income"]);if((0,e.cY)("[STRICT_SQL] GROUP BY - groupCol:",a,"valueCol:",b),a&&b){let c={},e=0;for(let f of d){let d=f[a]||"Unknown",g=parseFloat(f[b])||0;c[d]=(c[d]||0)+g,e+=g}let f=Object.entries(c).map(([a,b])=>({name:a,value:b,pct:e>0?b/e*100:0})).sort((a,b)=>b.value-a.value);j=`SELECT ${a}, SUM(${b}) as total FROM dataset GROUP BY ${a}`,k={type:"group_by",groupBy:a,value:b,data:f,operation:"group_by"}}}else if(g.includes("minimum")||g.includes("maximum")||g.includes("lowest")||g.includes("highest")){let a=l(["revenue","sales","profit","amount","price","cost","quantity","units"]);if(a){let b=d.map(b=>parseFloat(b[a])||0),c=Math.min(...b),e=Math.max(...b),f=g.includes("minimum")||g.includes("lowest");j=`SELECT ${f?"MIN":"MAX"}(${a}) as result FROM dataset`,k={[f?"minimum":"maximum"]:f?c:e,column:a,operation:f?"min":"max"}}}else if(g.includes("profit")&&(g.includes("margin")||g.includes("percentage"))){let a=l(["revenue","sales","amount"]),b=l(["cost","unit_cost"]);if(a&&b){let c=0,e=0;for(let f of d)c+=parseFloat(f[a])||0,e+=parseFloat(f[b])||0;let f=c>0?(c-e)/c*100:0;j="SELECT ((SUM(revenue) - SUM(cost)) / SUM(revenue)) * 100 as margin FROM dataset",k={profitMargin:f,revenue:c,cost:e,operation:"margin"}}}if((0,e.cY)("[STRICT_SQL] Generated SQL:",j),(0,e.cY)("[STRICT_SQL] Result:",JSON.stringify(k)?.slice(0,200)),!j||!k)return{success:!1,error:"Could not generate SQL for this question type"};return{success:!0,sql:j,result:k}}catch(a){return(0,e.AO)("[STRICT_SQL] Error:",a.message),{success:!1,error:a.message}}}async function r(a){if(!a)return{valid:!1,error:"No datasetId provided"};let b=await h.db.query.datasets.findFirst({where:(0,m.eq)(i.datasets.id,a)});return b?{valid:!0,dataset:b}:{valid:!1,error:"Dataset not found"}}[f,h,l]=o.then?(await o)():o;let z=new Map;function s(a,b,c={}){let d={...b,...c,action:a,timestamp:new Date().toISOString(),...c.question&&{question:c.question.slice(0,200),isAnalytical:/\b(how many|how much|total|sum|count|average|avg|top|highest|lowest|minimum|maximum|revenue|profit|region|currency|list|distinct|group by|analyze)\b/i.test(c.question)},...c.sql&&{sql:c.sql.slice(0,500)},...void 0!==c.executionTime&&{executionTimeMs:c.executionTime},...void 0!==c.success&&{success:c.success}};(0,e.cY)(`[CHAT] ${a}:`,JSON.stringify(d))}function t(a,b){for(let c of({country:[/country/i,/nation/i,/market/i],region:[/region/i,/continent/i,/area/i,/zone/i],product:[/product/i,/item/i,/sku/i,/goods/i,/merchandise/i],channel:[/channel/i,/source/i,/medium/i,/platform/i],category:[/category/i,/type/i,/segment/i,/industry/i],revenue:[/revenue/i,/sales/i,/amount/i,/total/i,/income/i,/value/i],quantity:[/quantity/i,/qty/i,/units/i,/count/i,/orders/i]})[b]){let b=a.find(a=>c.test(a));if(b)return b}return null}function u(a,b,c){let d={};for(let e of a){let a=e[b]||"Unknown",f=x(e[c]);d[a]=(d[a]||0)+f}return Object.entries(d).map(([a,b])=>({name:a,value:b})).sort((a,b)=>b.value-a.value).slice(0,10)}function v(a){return a>=1e6?`${(a/1e6).toFixed(2)}M`:a>=1e3?`${(a/1e3).toFixed(2)}K`:`${a.toLocaleString("en-US",{maximumFractionDigits:2})}`}function w(a){return`${a.toFixed(2)}%`}function x(a){if(null==a||""===a)return 0;if("number"==typeof a)return a;let b=String(a).replace(/[€$¥£C$A₹CHF₽]/g,"").replace(/\s/g,""),c=b.lastIndexOf("."),d=b.lastIndexOf(",");b=d>c?b.replace(/\./g,"").replace(",","."):c>d&&-1!==d?b.replace(/,/g,""):-1!==d&&-1===c?b.replace(",","."):b.replace(/,/g,"");let e=parseFloat(b);return isNaN(e)?0:e}async function y(a){let b=null;try{let{messages:c,datasetId:d,processedData:o}=await a.json();if(!c||!Array.isArray(c))return n.NextResponse.json({success:!1,error:"Invalid request: messages array required"},{status:400});let y=c[c.length-1]?.content||"",A=/\b(how many|how much|total|sum|count|average|avg|top|highest|lowest|minimum|maximum|revenue|profit|region|currency|list|distinct|group by|analyze)\b/i.test(y);if(A&&!d)return(0,e.cY)("[CHAT] REJECTED: Analytical query without datasetId"),n.NextResponse.json({success:!1,error:"No active dataset selected or invalid dataset ID",reason:"Please select an active dataset before asking analytical questions"},{status:400});let B=await (0,f.j2)(),C=B?.user?.id;if(C&&!(0,g.ub)(C)){let a=await (0,l.vD)(C);if(a.limitReached)return(0,e.cY)("[CHAT] REJECTED: Free limit reached"),n.NextResponse.json({success:!1,error:"Free limit reached",message:"You've used your 2 included Analyst credits. Subscribe to Pro or top up your balance to continue.",upgradeRequired:!0,analysisCount:a.analysisCount,creditsRemaining:0},{status:403})}if(d){(0,e.cY)("[CHAT] Validating datasetId:",d);let a=await h.db.query.datasets.findFirst({where:(0,m.eq)(i.datasets.id,d)});if(!a)return(0,e.cY)("[CHAT] REJECTED: Dataset not found:",d),n.NextResponse.json({success:!1,error:"No active dataset selected or invalid dataset ID",reason:"Dataset not found"},{status:400});(0,e.cY)("[CHAT] Dataset validated:",a.name,"- rows:",a.rowCount)}else(0,e.cY)("[CHAT] No datasetId - non-analytical query allowed");let D=d||"no-dataset";(0,e.cY)("[CHAT] Incoming message:",y),(0,e.cY)("[CHAT] Dataset ID:",d);let E=function(a,b){let c=Date.now(),d=z.get(a);if(d){if(c-d.lastTime>6e4)return z.set(a,{count:1,lastTime:c,lastMessage:b}),{allowed:!0};if(d.lastMessage===b&&d.count>=5)return{allowed:!1,message:"Chat blocked: Same message repeated 5+ times. Please rephrase your question."};z.set(a,{count:d.count+1,lastTime:c,lastMessage:b})}else z.set(a,{count:1,lastTime:c,lastMessage:b});return{allowed:!0}}(D+":"+y.slice(0,50),y);if(!E.allowed)return s("LOOP_DETECTED",{sessionKey:D,message:y.slice(0,50)}),n.NextResponse.json({success:!1,error:E.message},{status:429});s("AI_CALL_INITIATED",{datasetId:d,messageLength:y.length});let F=A||(0,j.od)(y);if(d&&F){(0,e.cY)("[CHAT] Question requires verified computation (analytical detected:",A,", computation:",(0,j.od)(y),")");let a=await r(d);if(!a.valid)return n.NextResponse.json({success:!1,error:"No active dataset selected or invalid ID",reason:a.error},{status:400});(0,e.cY)("[STRICT_SQL] Executing strict SQL for:",y);let b=await q(d,y);if(!b.success){(0,e.cY)("[STRICT_SQL] Failed:",b.error);let a=await h.db.query.datasets.findFirst({where:(0,m.eq)(i.datasets.id,d)}),c=a?.columns||[];return n.NextResponse.json({error:"Unable to compute this question from the dataset",reason:b.error||"No matching computation pattern found",availableColumns:c,suggestion:"Try asking about: total revenue, average sales, count of rows, top products by revenue, or rephrase your question to match available columns: "+c.slice(0,10).join(", ")},{status:400})}(0,e.cY)("[STRICT_SQL] Success! Result:",JSON.stringify(b.result).slice(0,200));let c=(0,k.vw)({success:!0,computed_value:b.result.count||b.result.total||b.result.average||b.result.data||b.result.minimum||b.result.maximum||b.result.profitMargin||0,operation:b.result.operation,row_count:a.dataset?.rowCount},y),f=await fetch("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${process.env.GEMINI_API_KEY}`},body:JSON.stringify({model:"gemini-2.5-flash",messages:[{role:"system",content:k.ZF},{role:"user",content:c}],temperature:.3,max_tokens:500})});if(f.ok){let a=await f.json(),c=a.choices?.[0]?.message?.content||"",d=p(c);return n.NextResponse.json({success:!0,content:d,role:"assistant",verified:!0,computation:{operation:b.result.operation,sql:b.sql,result:b.result}})}}let G=null,H=[];if(o&&Array.isArray(o)&&o.length>0)(0,e.cY)("[CHAT] Using processed data from frontend:",o.length,"rows"),H=o.slice(0,50);else if(d){(0,e.cY)("[CHAT] Fetching dataset from database...");let a=await h.db.query.datasets.findFirst({where:(0,m.eq)(i.datasets.id,d)});if(a){G={id:a.id,name:a.name,rowCount:a.rowCount,columnCount:a.columnCount,columns:a.columns},(0,e.cY)("[CHAT] Dataset found:",G.name,"-",G.rowCount,"rows");let b=a.data||[];H=(function(a){if(!a||0===a.length)return a;let b=a[0],c=Object.keys(b),d=/price|amount|revenue|cost|total|profit|sales|value|qty|quantity/i,f=c.filter(a=>d.test(a));return(0,e.cY)("[NORMALIZE] Detected monetary columns:",f),a.map(a=>{let b={...a};for(let c of f){let d=a[c];"string"==typeof d&&/[€$¥£C$A₹CHF₽]/.test(d)&&(b[c]=x(d),(0,e.cY)(`[NORMALIZE] ${c}: "${d}" -> ${b[c]}`))}return b})})(b).slice(0,50),(0,e.cY)("[CHAT] Fetched and normalized",H.length,"sample rows for context")}else(0,e.cY)("[CHAT] Dataset not found in database")}["generate report","create report","export report","download report","make pdf","create presentation","create powerpoint","create word document","export excel","investor report","management report","board report","executive summary document","export document","investor deck","branded report","executive presentation","detailed board report"].some(a=>y.toLowerCase().includes(a));let I=`You are Clevr, elite AI analyst for startup founders and investors.
...
Remember: Respond with TEXT ONLY. Do not execute any commands or tools.`,(0,e.cY)("[GEMINI] API Key present:",!!process.env.GEMINI_API_KEY),!process.env.GEMINI_API_KEY)return n.NextResponse.json({success:!1,error:"AI service not configured. Please contact support."});let J=await fetch("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${process.env.GEMINI_API_KEY}`},body:JSON.stringify({model:"gemini-2.5-flash",messages:[{role:"system",content:I},...c],temperature:.3,max_tokens:1500})});if((0,e.cY)("[GEMINI] Status:",J.status),!J.ok){let a=await J.text();return(0,e.AO)("[GEMINI ERROR]",J.status,a),n.NextResponse.json({success:!1,error:`AI service error: ${J.status}`},{status:J.status})}let K=await J.json(),L=K.choices?.[0]?.message?.content||"";return(0,e.cY)("[GEMINI] Response received:",L.slice(0,100)),s("AI_CALL_COMPLETE",{datasetId:d,responseLength:L.length}),b={success:!0,content:p(L),role:"assistant"},n.NextResponse.json(b)}catch(a){return(0,e.AO)("[CHAT CRASH]",{message:a.message,stack:a.stack?.slice(0,500)}),n.NextResponse.json({success:!1,error:a.message||"An unexpected error occurred. Please try again."},{status:500})}}d()}catch(a){d(a)}})},86439:a=>{a.exports=require("next/dist/shared/lib/no-fallback-error.external")}};var b=require("../../../webpack-runtime.js");b.C(a);var c=b.X(0,[4657,6796,817,4819,7841,4349,7700,6050],()=>b(b.s=52380));module.exports=c})();