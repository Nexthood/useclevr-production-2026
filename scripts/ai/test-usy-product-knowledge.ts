...
function testProAndBusinessClevrSyncEnabled() {
  for (const tier of ["pro", "business"] as const) {
    ...
    assert.match(reply.answer, /Available connectors: Google Sheets, OneDrive, and SharePoint\./)
  }
}
...
function testSuperadminClevrSyncIndependentOfSubscription() {
  ...
  assert.match(unlimitedUsage.answer, /Available connectors: Google Sheets, OneDrive, and SharePoint\./)
...
function testConnectorsListAllThreeAvailable() {
  const reply = buildUsyReply({
  ...
  assert.match(reply.answer, /The OneDrive flow is: Connect Microsoft/)
  assert.match(reply.answer, /The SharePoint flow is: Connect Microsoft/)
  assert.match(
    reply.answer,
...
function testClevrSyncAnswersStayLocalized() {
  ...
  assert.match(germanPro.answer, /Verfügbare Connectors: Google Sheets, OneDrive und SharePoint\./)
  assert.doesNotMatch(germanPro.answer, /kommen bald/)
  assert.match(germanPro.answer, /Google-Sheets-Ablauf/)
  assert.match(germanPro.answer, /OneDrive-Ablauf/)
  assert.match(germanPro.answer, /SharePoint-Ablauf/)
...
const tests = [
  ...
  { name: "Connector list names Google Sheets, OneDrive, and SharePoint as available", fn: testConnectorsListAllThreeAvailable },
