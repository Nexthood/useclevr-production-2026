...
const from = path.join(root, "dist-root", "server-config", "railway.json");
...
if (checkOnly) {
  if (!fs.existsSync(to)) {
    ...
    console.log("dist-root/server-config/railway.json is valid; dist/railway.json is generated");
    process.exit(0);
  }

  if (fs.readFileSync(from, "utf8") !== fs.readFileSync(to, "utf8")) {
    throw new Error("dist/railway.json is out of sync with dist-root/server-config/railway.json");
  }
