...
assert.match(wizardSource, /onClick=\{\(\) => setCompleted\(false\)\}/, "the edit action reopens the completed profile for editing")
assert.match(wizardSource, /View saved profile summary/, "completed Business Profile keeps the saved profile summary action")
assert.match(wizardSource, /Business Profile completed/, "completed Business Profile keeps its confirmation state")
assert.match(wizardSource, /\{completed \? \(/, "completed state renders the confirmation panel with the editable flow preserved underneath")
assert.match(wizardSource, /MultiChoiceWithCustom[\s\S]*customPlaceholder="Enter a business model\.\.\."/u, "business models keep custom free-text entry with suggestion chips")
assert.match(wizardSource, /premises or property costs/, "premises cost question stays generalized across business premises types")
assert.match(wizardSource, /if \(isOpen && !completed\) questionHeadingRef\.current\?\.focus\(\)/, "question heading focus stays on the active editor step")

assert.match(wizardSource, /fetch\("\/api\/business\/setup"[\s\S]*method: "PUT"/, "profile edits save through the existing setup API")
assert.match(wizardSource, /setCompleted\(savedPayload\.setupStatus\.completed\)/, "successful save updates the completed state from the persisted profile")
