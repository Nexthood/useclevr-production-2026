...
function testApiRouteWiring() {
  ...
  assert.ok(!source.includes('fetch("/api/usage/credits"'), "Accountancy upload UI does not pre-block on normal upload credits");
  assert.ok(source.includes("const isUploadBlocked = isPlanLimitReached"), "Accountancy upload UI only uses Accountancy-local plan states for local blocking");
  ...
  assert.ok(!processor.includes("buildUploadCreditLimitInlineMessage"), "Accountancy upload API does not generate normal upload-credit exhaustion copy");
