...
| --- | --- | --- | --- | --- |
| I-001 | Railway test host can return runtime 502 while local packaging succeeds. | Technical owner | Open | Check Railway logs, environment, and `/api/health` after the next deploy. |
| I-002 | Sales docs need a current demo dataset and screenshot set. | Sales owner | Open | Prepare founder and SME demo datasets and capture current UI screenshots. |
...
| I-005 | Payload CMS planning is ready but not active implementation. | Product owner | Controlled | Keep Payload migration in `.TODO/todo-migration-payplod.md` until activated. |
