...
- **Durable change**: Dataset AI now keeps the selected dataset ID and authenticated user context through deterministic answers, wraps saved provider settings safely, sends provider-backed selected-dataset questions to the configured Gemini key or Antigravity cloud AI, and reports sanitized provider failure classes when the cloud provider is unavailable.
