...
- **Durable change**: Dataset AI now keeps the selected dataset ID and authenticated user context through deterministic answers, wraps saved provider settings safely, and sends provider-backed selected-dataset questions to the configured Gemini key or Antigravity cloud AI when saved provider setup is unavailable or no saved provider handles the request.
