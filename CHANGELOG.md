...
- Fixed `dist/package.json` generated by `scripts/dist/create-dist.cjs` writing `"type": "commonjs"`
  while `dist/server.js` is an ESM file. The mismatch produced
  `Cannot use import statement outside a module` on Railway and caused `/api/health` to timeout.
  `create-dist.cjs` now writes `"type": "module"` so `node server.js` processes the ESM entry
  without error.
...
- Added `tscAndConditionsUrl` field to billing plans and set it to `https://useclevr.com/terms` on
  all paid plans.
- Added `GET /api/checkout/options` — returns plan list with `tscAndConditionsUrl`, `status`, and
  `paymentProviderConnected`.
- Added `POST /api/checkout/confirm` honours `?form=review-accepted` and returns a Stripe-ready
  status when the payment provider is connected, enabling the two-step checkout flow.
...
- Added Stripe billing columns (`stripeCustomerId`, `stripeSubscriptionId`, `stripePriceId`,
  `stripeStatus`, `stripeCurrentPeriodEnd`) to the `profiles` table schema.
- Added business details columns (`businessName`, `businessEmail`, `industry`, `location`,
  `website`, `businessDescription`) to the `profiles` table schema.
- Added `GET /api/me/business` — returns the current user's business profile details.
- Added 2-step checkout flow (Step 1 "Review plan" → Step 2 "Terms & conditions" with required
  checkbox acceptance).
- Added Business settings page (`/app/settings/business`) with a six-field form, live completion
  percentage, and save feedback.
- Added Payment settings page (`/app/settings/payment`) showing `STRIPE_SECRET_KEY` /
  `STRIPE_WEBHOOK_SECRET` env-var status.
- Added `updateBusinessDetails` server action to persist business profile fields to the database.

### Changed

- Added Business entry (all users) and Payment entry (superadmin) to the settings navigation
  sidebar.
- Updated topbar to show the full plan name (e.g. `Pro · Pro`) and a business profile completion
  badge linking to `/app/settings/business`.
- Standardized visible product branding as UseClevr across UI, docs, metadata, and generated package
  naming.

### Removed

- Removed `src/components/modals/upgrade-modal.tsx` — replaced by the shared `Modal` component.

### Refactored

- Added reusable `Modal` component (`src/components/ui/modal.tsx`) with `fullscreen` variant and
  `createPortal`-based body scroll handling.
- Replaced `UpgradeModal` with the shared `Modal` component in `DownloadsPage`.
- Migrated `MegaInstallerModal` and `DatasetModal` to use the shared `Modal` component (with
  `fullscreen` variant where appropriate).
- Refactored `hybrid-ai-button.tsx` to own its fullscreen `createPortal` instead of depending on a
  separate popup file.

### Security

- Removed inline Ollama/runtime model naming from all product-display copy so that no infrastructure
  or model name is visible to end users.
