...
`next build --webpack` always produces a new compilation hash. `create-dist.cjs` wipes and rewrites `dist/` from scratch with no content hashing. This is expected; both `dist/` and `.next/` are `.gitignore`-ed and regenerated by CI on every push. CI activates pnpm through Corepack so installs do not depend on a third-party pnpm setup action.
