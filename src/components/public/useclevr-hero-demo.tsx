...
export function UseClevrHeroDemo({ id, className = "", layout = "landing" }: UseClevrHeroDemoProps) {
  const demoGridClass =
    layout === "auth"
      ? "grid gap-4 xl:grid-cols-[0.92fr_1.08fr]"
  ...
  return (
    <Card
      id={id}
      className={[
        "relative overflow-hidden border-white/10 bg-slate-950 p-4 text-left text-white shadow-2xl shadow-cyan-950/20 backdrop-blur md:p-6",
        className,
      ].filter(Boolean).join(" ")}
    >
