  ...
  await db.transaction(async (tx) => {
    await tx.update(userCredits)
      .set({
        ...
        usedCredits: 0,
        reservedCredits: 0,
        remainingCredits: monthlyCredits + (creditInfo.purchasedBalance ?? 0),
    ...
    await tx.insert(creditLedger).values({
      ...
      credits: monthlyCredits,
      balanceBefore: creditInfo.remainingCredits,
      balanceAfter: monthlyCredits + (creditInfo.purchasedBalance ?? 0),
    ...
    await db.transaction(async (tx) => {
      await tx.update(userCredits)
        .set({
          ...
          remainingCredits: Math.max(0, creditInfo.includedBalance + creditDiff) + (creditInfo.purchasedBalance ?? 0),
          creditsResetAt: nextReset,
      ...
      await tx.insert(creditLedger).values({
        ...
        balanceBefore: creditInfo.remainingCredits,
        balanceAfter: Math.max(0, creditInfo.remainingCredits + creditDiff) + (creditInfo.purchasedBalance ?? 0),
        includedBalanceBefore: creditInfo.includedBalance,
