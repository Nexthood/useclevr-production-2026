...
function buildAccountantCsvRows(rows: CategorizedTransaction[]) {
  return rows.map((transaction) => ({
    ...
    "VAT Confidence": typeof transaction.vatConfidence === "number" ? `${Math.round(transaction.vatConfidence * 100)}%` : "",
    "VAT Reason": transaction.vatReason || "",
    "VAT Rule": transaction.vatBusinessRule || "",
    "VAT Source": transaction.vatSource || "",
