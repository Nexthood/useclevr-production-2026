...
function drawRetailSalesPerformance(doc: jsPDF, report: Report, financials: ReportFinancials) {
  ...
  y = drawTable(doc, [
    ...
    ["Average Order Value", report.kpis.find((kpi) => kpi.title === "AOV")?.value || "Not available", report.kpis.some((kpi) => kpi.title === "AOV") ? "Available" : "Not available", "Requires order ID or row-level transaction grain."],
  ...
  if (margins.length === 0) {
    ...
    drawTable(doc, [
      ...
      ...margins.slice(0, 8).map((item): TableRow => [
        ...
        "Derived from category revenue and cost.",
      ]),
