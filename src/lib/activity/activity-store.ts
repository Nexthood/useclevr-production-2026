...
export async function listUserActivities(userId: string, limit = MAX_USER_ACTIVITIES): Promise<ProductActivity[]> {
  const db = getDb()
  if (!db) return []
...
async function pruneUserActivities(userId: string) {
  ...
  await db.delete(userActivities).where(inArray(userActivities.id, stale.map((item) => item.id)))
}
