...
async function main() {
  console.log("=== Testing Revenue Upload ===")
  try {
  ...
  console.log("\n=== Testing Expenses Upload ===")
  try {
    const result = await uploadCSV(createFormData('expenses', expenseCSV, 'expenses.csv'))
    console.log("Expenses upload result:", JSON.stringify(result, null, 2))
  } catch (error) {
    console.error("Expenses upload exception:", error)
    console.error("Stack:", error.stack)
  }
}
