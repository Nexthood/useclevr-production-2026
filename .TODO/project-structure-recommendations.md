...
This document consolidates the previous project structure audit notes into one future-facing recommendation file.
...
- `lib/` contains business logic, data processing, AI adapters, storage, database access, and utilities.
- `docs/` contains team documentation, setup guides, flowcharts, requirements, and recommendations.
...
- `components/layout/` for shell, header, footer, and sidebar components.
...
- Unit tests for pure analytics, formatting, query validation, and forecast helpers.
- Integration tests for high-value API routes.
- A small Playwright smoke test for upload, analyze, chat/query, and report flows.

Do not enable a strict type-check gate until current strict-mode issues are fixed.

## Deployment Notes

- Keep Railway commands aligned with `package.json` and `railway.json`.
...
- Monitor generated report and upload storage size in ephemeral containers.
...
- Direct `process.env` usage in browser components.

## Suggested Next Steps

1. Add a service boundary around report generation and dataset analysis.
2. Add a basic test baseline for pure data transforms and high-value API routes.
3. Centralize runtime configuration validation.
4. Reorganize components by feature as files are touched.
5. Add architecture decision records when major structure choices are made.
