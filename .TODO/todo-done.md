...
- T-1066. The dashboard World Map aggregates the same row scope as the KPI row: semantic-KPI datasets feed the map every stored row through one shared scope selector instead of the default 30-day window, fallback-KPI datasets keep the range-filtered scope on both sides, and a regression suite pins the 220-row/42-row fingerprint with map totals equal to KPI totals. (labels: dashboard, data, metrics; commit: worktree)
...
- T-1070. E-commerce reports and dashboards recognize explicit Profit and safe retail Cost fields: canonical COGS, gross profit, and gross margin feed report KPIs, recommendations, and the e-commerce dashboard, explicit Profit takes precedence over derived revenue-minus-cost, shipping/marketing/operating costs never classify as COGS, missing metrics stay unavailable, and ClevrSync plus normal CSV/XLSX uploads share the same central semantics. (labels: reports, data, metrics, testing; commit: worktree)
...
- T-1010. Detect strict accountancy ledger schemas during generated-report model resolution so standard uploads with debit, credit, and account or journal evidence route to ledger reporting. (labels: reports, data, testing; commit: 3e747a47)
...
- T-1074. Usy ClevrSync knowledge serves the final connector state: Google Sheets is the only available connector according to the plan entitlement, CSV and Excel are described as normal direct uploads, every tier and all six languages state that OneDrive and SharePoint are not part of the product, and the required connector, Excel-upload, and Google Sheets questions route to product knowledge instead of the contact request flow or upload troubleshooting, pinned by 22 product-knowledge regression tests. (labels: ai, ui, testing; commit: worktree)
...
- T-1073. ClevrSync offers Google Sheets only: the Microsoft OneDrive and SharePoint connectors, their OAuth routes, Graph connector service, token/state stores, connector cards, entitlement flags, env-variable examples, and Microsoft test suites are removed without disabled leftovers, while Google Sheets, direct CSV/Excel upload, upload security, credits, and dataset isolation stay intact. (labels: upload, data, api, testing; commit: worktree)
...
- T-1071. Upload and document ingestion verifies server-side what every file really is: one central validator checks the normalized filename, the full extension chain, magic bytes, and declared MIME before any parser runs, blocking double-extension HTML phishing files like `Payment Slip.pdf.html` on the standard, simple, Accountancy, Pre-bookkeeping, bank, receipt, PDF, ClevrSync, MCP, and dataset-file ingestion paths, with safe structured `UNSAFE_FILE_TYPE` and `FILE_TYPE_MISMATCH` errors, security rejection logging, and formula-injection-neutral CSV exports. (labels: security, upload, data, testing; commit: worktree)
...
- T-1064. Fix the Risk Intelligence production RangeError so workspace listing and initial dataset determination format dataset metadata timestamps through canonical validation instead of calling Date.toISOString on invalid Date values, return null timestamps for invalid or missing metadata while keeping the dataset selectable, harden the page last-calculated rendering with canonical validation, and pin invariant tests that forbid toISOString execution on Invalid Dates across the whole Risk Intelligence path. (labels: business, ai; commit: worktree)

## Label: data

- T-1072. Add OneDrive and SharePoint ClevrSync connectors on one shared Microsoft OAuth + Microsoft Graph infrastructure: implement Microsoft token vault reuse through the central ClevrSync vault, HMAC state with nonce replay protection, incremental consent where SharePoint site scopes are added only on SharePoint connect, least-privilege delegated read-only Graph scopes, OneDrive workbook discovery, SharePoint site/library/workbook discovery, shared worksheet discovery, canonical worksheet preview without credits, Connect & Analyze through the central 10-credit upload+analysis operation, Sync now that re-fetches the persisted site/library/workbook/worksheet identity and refreshes the linked dataset without new charges, Graph throttling with bounded Retry-After-aware retries, onedrive/sharepoint dataset source provenance, Data Connections cards and panels showing all three connectors as Available, Usy product knowledge for all three connectors, and regression suites for OAuth state, discovery, credits, and isolation. (labels: data, api, auth, billing, ui, security; commit: worktree)
  Superseded the same day by T-1073: the Microsoft ClevrSync integration was removed completely.
