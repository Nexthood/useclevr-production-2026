...
- Profile organizational maturity with the Enterprise Dataset Intelligence business maturity engine by using business-model context when available, dataset size, entity count, relationship density, financial complexity, inventory complexity, store count, warehouse count, department count, currency usage, country presence, business vocabulary, historical data availability, data quality, confidence scoring, evidence, warnings, growth-stage detection, readiness scores, and maturity statistics.
- Keep the Enterprise Dataset Intelligence business maturity engine free of business-model classification, KPI discovery, dashboard personalization, recommendation generation, forecasting, investor assessment, benchmarking, ESG assessment, IPO assessment, M&A assessment, compliance assessment, risk and opportunity generation, active learning, and human-review workflow execution until those EDIE phases explicitly add those responsibilities.
- Discover relevant KPI candidates with the Business Intelligence universal KPI discovery engine by using EDIE structure, semantic, entity, relationship, business-model, and business-maturity outputs, configurable KPI definitions, dependency-aware availability, confidence scoring, evidence, category coverage, missing-data identification, plugin registration, quality scoring, and profile statistics.
- Keep the Business Intelligence universal KPI discovery engine free of dashboard rendering, KPI value calculation, recommendation generation, forecasting, what-if analysis, scenario planning, benchmarking, ESG scoring, investor scoring, banking scoring, IPO scoring, learning, active validation, graph analytics, streaming updates, and incremental KPI execution until later BIE phases explicitly add those responsibilities.
- Compose generated dashboard profiles with the Business Intelligence universal dashboard composer by using business-model signals, business maturity, KPI discovery, entity intelligence, relationship intelligence, dataset quality, widget library definitions, widget selection evidence, responsive layout rules, section priorities, confidence scoring, missing-data warnings, and dashboard statistics.
- Keep the Business Intelligence universal dashboard composer free of UI rendering, user personalization, drag-and-drop layout editing, interactive drill-down, cross-filtering, dashboard sharing, dashboard export, scheduled reports, embedded dashboards, live widgets, collaboration, dashboard comments, marketplace packs, white-label dashboards, mobile-app dashboards, AR/VR dashboards, KPI value calculation, and recommendation generation until later BIE phases explicitly add those responsibilities.
- Generate evidence-backed insight profiles with the Business Intelligence universal insight generation engine by using business-model signals, business maturity, relationship graph context, KPI discovery, entity statistics, semantic coverage, dataset quality, confidence scoring, priority scoring, duplicate and overlap detection, grouped insight categories, investigation guidance, logs, and statistics.
- Keep the Business Intelligence universal insight generation engine free of AI-written narratives, natural-language reports, personalized insight delivery, scheduled delivery, predictive insights, root-cause analysis, what-if analysis, simulations, recommendations, alerts, mobile notifications, chat integrations, email summaries, voice assistants, multi-language generation, and historical insight tracking until later BIE phases explicitly add those responsibilities.
- Generate evidence-backed recommendation profiles with the Business Intelligence universal recommendation engine by using business-model signals, business maturity, relationship graph context, KPI discovery, insight profiles, business health, risk indicators, confidence scoring, priority scoring, implementation difficulty, estimated benefit, required data, dependency mapping, duplicate and conflict detection, logs, and statistics.
- Keep the Business Intelligence universal recommendation engine free of AI decision execution, automated advisory workflows, workflow automation, ERP integration, CRM integration, POS integration, email delivery, Slack or Teams notifications, scheduled recommendations, recommendation learning, user-feedback learning, ROI tracking, success tracking, action confirmation, continuous optimization, benchmark comparison, and industry benchmark execution until later BIE phases explicitly add those responsibilities.
- Generate explainable forecast and scenario profiles with the Business Intelligence universal forecast and scenario engine by using historical data, KPI discovery, business-model signals, business maturity, insight profiles, recommendation profiles, relationship graph context, seasonality evidence, business rules, forecast model registration, confidence intervals, uncertainty warnings, scenario simulation, logs, and statistics.
- Keep the Business Intelligence universal forecast and scenario engine free of real-time forecasting, Monte Carlo simulation, digital twins, economic indicator ingestion, weather ingestion, competitor signals, AI self-learning, external API execution, supply-chain event ingestion, dynamic pricing execution, investment planning, capacity planning, workforce planning, multi-year forecasting, and strategic planning until later BIE phases explicitly add those responsibilities.
...
- Rebuild legacy idempotent generated reports when stored report metadata lacks current row-use diagnostics or semantic context, so repeated report requests cannot return stale PDF content.
- Select a dataset-aware generated report profile for standard report generation, including local retail, e-commerce, SaaS startup, marketplace startup, investor portfolio, business consulting, professional services, generic business, profitability P&L, and accountancy ledger profiles.
- Display the SaaS Executive Report reporting period from the minimum and maximum recognized source periods used by SaaS latest-period and trend metrics; display one recognized period for single-period datasets and "Not available" only when the selected dataset has no valid recognized period.
- Limit the SaaS Executive Report final Results Summary data-status messaging to SaaS capability gaps such as plan, geography, or support-ticket fields, and do not request generic Revenue, COGS, Gross Profit, or Operating Expenses fields when SaaS-specific analysis is available.
- Render SaaS and startup standard uploads with source-backed unit-economics semantics: preserve company, plan, users, price per user, revenue, cost, profit, startup stage, country, and date fields; calculate profit from source profit or revenue minus cost; calculate profit margin and average revenue per user only from available columns; and exclude retail inventory, order-value, MRR, ARR, CAC, LTV, churn, burn, and runway metrics unless the uploaded dataset provides deterministic source fields.
...
- Persist paired Profitability uploads as one parent Profitability analysis that uses the stable parent analysis identifier, attaches the revenue and expense files as source inputs, stores combined source-row counts, and routes Generate Profitability Analysis, report generation, report opening, PDF downloads, Excel downloads, refreshes, and Dashboard context to the parent analysis instead of either child input file.
- Render the existing rich Profitability analytics dashboard as the final view for active paired Revenue plus Expense analyses after async processing, navigation, and refresh; the view uses the active parent Profitability analysis before child dataset state and auto-detected dataset profiles, and its primary profit KPIs show canonical Revenue, Operating Expenses, Operating Profit, and Operating Margin.
- Keep paired Profitability dashboards on the canonical Profitability profile after async processing, refetch, refresh, navigation, and tab changes; use Revenue, Operating Expenses, Operating Profit, Operating Margin, Profitability Daily Health findings, Profitability Balanced Scorecard inputs, Profitability recommendations, and Profitability role labels for revenue and expense child files instead of child-file Marketplace, E-Commerce, inventory, seller, buyer, GMV, or low-stock semantics.
- Render local retail generated reports as Retail Executive Reports with revenue, COGS, gross profit, gross margin, and units sold from transaction rows; calculate current stock, inventory value, low-stock positions, reorder-required positions, out-of-stock positions, and stock by category from the latest store-product inventory snapshots; keep product or SKU count as distinct product IDs; show category gross margin that reconciles to total revenue and COGS, average order value only when reliable order identifiers or explicit order-level transaction grain exist, inventory intelligence, product/category/supplier intelligence, retail recommendations, and provenance.
- Prioritize local retail report recommendations around stockout and reorder risk, inventory cash exposure, weak product or category margins, supplier concentration, sales deterioration signals, dead or excess stock where supported, and missing operational fields; do not lead retail reports with generic missing interest, tax, or operating-expense P&L advice.
...
- Read Accountancy Business Profile context through the same saved Business Profile repository used by the Business page and display tax country, currency, fiscal year, VAT or sales tax, payroll, and fixed costs from that persisted profile only.
- Show Accountancy Business Profile loading, failure, and missing-field states separately so load failures never render as configured-field gaps.
...
- Resolve selected-dataset Marketplace revenue-worded questions with Marketplace semantics: label gross merchandise value as GMV, keep platform fee as Marketplace Revenue, group buyer questions by buyer fields, group seller or supplier-worded questions by seller/merchant fields, include the latest observed period in GMV trends, and render missing inventory/stock evidence as unavailable instead of zero.
