...
- Render generic business generated reports with reliable invoice IDs as transaction identifiers when stronger order or transaction IDs are absent, exact `cost` fields as generic business cost totals, explicit `profit` fields as source-backed profit, profit margin as profit divided by revenue, and no net-profit label for generic `profit` fields.
...
- Render local retail generated reports as Retail Executive Reports with revenue, COGS, gross profit, gross margin, units sold, current stock, inventory value, product or SKU count, low-stock SKUs, reorder-required SKUs, out-of-stock SKUs, category gross margin that reconciles to total revenue and COGS, average order value only when reliable order identifiers or explicit order-level transaction grain exist, inventory intelligence, product/category/supplier intelligence, retail recommendations, and provenance.
